  ...
  <v-card border flat class="fill-card">
    ...
    <div ref="tableArea" class="fill">
      <v-data-table-virtual :headers="headers" :items="items" :loading="loading" item-value="id" class="consumable-table"
                            density="compact" fixed-header :height="tableHeight" no-data-text="No holding records.">
        ...
        <template #item.ovenType="{ item }">
          <v-chip v-if="item.isFinishedAfterBaking" size="small" color="info" variant="tonal" label>Finished After Baking</v-chip>
          <span v-else>{{ item.ovenType || '—' }}</span>
        </template>
        <template #item.compartmentNumber="{ item }">
          <v-chip v-if="item.compartmentNumber" size="small" color="indigo" variant="tonal" label>{{ item.compartmentNumber }}</v-chip>
...
<script setup>
  ...
  const headers = [
    ...
    { title: 'Consumable', key: 'diaSpec', width: '19%' },
    { title: 'Oven Type', key: 'ovenType', width: '12%' },
    { title: 'Compartment', key: 'compartmentNumber', align: 'center', width: '8%' },
    { title: 'KG', key: 'quantityKg', align: 'end', width: '6%' },
    { title: 'Welder Name', key: 'welderName', width: '11%' },
    { title: 'Remarks', key: 'remarks', width: '6%' },
  ...
  function exportCsv() {
    const header = ['Holding No', COLUMN.date, 'Welder Name', 'Baking No', COLUMN.diaSpec, COLUMN.brand, COLUMN.lot,
      'Oven Type', 'Compartment', 'Finished After Baking', 'Quantity (KG)', 'Remarks', 'Voided', 'Entered by']
    const rows = items.value.map((h) => [h.holdingNo, fmtDate(h.holdingDate), h.welderName ?? '', h.bakingNo, h.diaSpec, h.brand,
      h.lotNumber, h.ovenType ?? '', h.compartmentNumber ?? '', h.isFinishedAfterBaking ? 'Yes' : '', Number(h.quantityKg).toFixed(2),
